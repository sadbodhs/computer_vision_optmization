...
**From a clean clone** (rebuilds images, models, and the replay input — full
recipe in [`docker/README.md`](docker/README.md)):
...
**Individual arms** (server already running):
...
Engines and `frames.bin` are gitignored (GPU-specific / large) and regenerated by
`scripts/export_models.sh` and `scripts/make_frames.sh` respectively. The two
container images rebuild from source via `docker/build.sh` — no reliance on a
committed image surviving.
